...
config, ... }: {
  ...
  home.file = {
    ...
    # doesn't work on mac, split out to linux specific
    # ".emacs.d".source = config.lib.file.mkOutOfStoreSymlink (lib.snowfall.fs.get-file "emacs.d"");
    ...
    # doesn't work on mac, split out to linux specific
    # ".vim".source = config.lib.file.mkOutOfStoreSymlink (lib.snowfall.fs.get-file "vim");
